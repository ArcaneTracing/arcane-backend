...
describe("Evaluations – experiment statistics performance (E2E)", () => {
  ...
  describe.each(testSizes)("Dataset size: %d entries", (size) => {
    it("nominal statistics - correctness and performance", async () => {
      ...
      const distribution = { "1": 0.4, "2": 0.3, "3": 0.2, "4": 0.1 };
      ...
      log(`Seeding experiment evaluation (${size} rows)...`);
      ...
      const nominalStat = result.body.find(
    ...
    it("numeric statistics - correctness and performance", async () => {
      ...
      expect(result.status).toBe(200);
      const numericStat = result.body.find(
    ...
    it("ordinal statistics - correctness and performance", async () => {
      ...
      expect(result.status).toBe(200);
      const ordinalStat = result.body.find(
        (s: any) => s.experimentId === experimentId && s.ordinal,
      );
