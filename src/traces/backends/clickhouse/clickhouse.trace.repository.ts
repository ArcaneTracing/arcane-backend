...
export class ClickHouseTraceRepository implements TraceRepository {
  ...
  public getQualifiedTableRef(datasource: Datasource): string {
    const config = this.getClickHouseConfig(datasource);
    return ClickHouseQueryBuilder.buildQualifiedTableRef(
      config.tableName,
      config.database,
    );
  }
  ...
  async getAttributeNames(datasource: Datasource): Promise<string[]> {
    try {
      const client = this.getClient(datasource);
      const config = this.getClickHouseConfig(datasource);
      const qualifiedTableRef = ClickHouseQueryBuilder.buildQualifiedTableRef(
        config.tableName,
        config.database,
      );
