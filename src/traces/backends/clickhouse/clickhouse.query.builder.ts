...
export class ClickHouseQueryBuilder {
  ...
  /**
   * Builds a database-qualified table reference for ClickHouse.
   * Uses database.tableName format when database is provided to fix
   * "Unknown table expression identifier" errors in CTEs and distributed queries.
   */
  public static buildQualifiedTableRef(
    tableName: string,
    database?: string,
  ): string {
    if (database) {
      return `${this.escapeIdentifier(database)}.${this.escapeIdentifier(tableName)}`;
    }
  ...
  static buildSearchQuery(
    ...
    const qualifiedTableRef = this.buildQualifiedTableRef(tableName, database);
    const query = `
