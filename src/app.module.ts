  ...
  imports: [
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (configService: ConfigService) => {
        ...
        logger.log(
          `NODE_ENV=${nodeEnv ?? "undefined"} (process.env.NODE_ENV=${process.env.NODE_ENV ?? "undefined"}), synchronize=${isLocal}`,
        );
        ...
        logger.log(`Connecting to database: ${url.hostname}:${url.port}/${url.pathname.slice(1)}`);
