...
export class TempoConversationRepository implements ConversationRepository {
  ...
  private coerceValueToString(value: unknown): string {
  ...
  private escapeTraceQLStringValue(value: string): string {
  ...
  private buildAttributeValueQuery(
    ...
    const strValue = this.coerceValueToString(value);
    const escaped = this.escapeTraceQLStringValue(strValue);
    const conditions = attributes.map(
      (attr) => `span."${attr}" = "${escaped}"`,
    );
    return `{ ${conditions.join(" || ")} }`;
  }
