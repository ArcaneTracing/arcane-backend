...
describe("TempoParamsBuilder", () => {
  ...
  describe("build", () => {
    ...
    it("should preserve backslash-escaped quotes in query when encoding", () => {
      const queryWithEscapedQuotes =
        'span."operator.metadata.chatId" = "\\"bcbaa99c-5f7e-4c4c-bd8a-dd6fcbe1a7ae\\""';
      const searchParams: SearchTracesRequestDto = {
        start: "2024-01-01T00:00:00Z",
        end: "2024-01-02T00:00:00Z",
        q: queryWithEscapedQuotes,
      };

      const params = TempoParamsBuilder.build(searchParams);
      const queryString = params.toString();

      expect(queryString).toContain("q=");
      const qParam = queryString.split("q=")[1]?.split("&")[0] ?? "";
      const decoded = decodeURIComponent(qParam);
      expect(decoded).toBe(queryWithEscapedQuotes);
